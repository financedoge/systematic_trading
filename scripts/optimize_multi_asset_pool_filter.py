...
from systematic_trading.backtest.comparison import compare_backtests  # noqa: E402
...
_WORKER_SPLIT_DATE: date | None = None
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Optimize multi-asset asset-pool filter parameters on the pre-split window, "
            "then evaluate the frozen winner out-of-sample."
        )
    )
    ...
    parser.add_argument("--output-dir", default="var/backtests/multi_asset_pool_filter_optimization_2012")
    ...
    _score_rows(rows)
    ranked = sorted(rows, key=lambda row: row["inSampleObjectiveScore"], reverse=True)
    winner = ranked[0]
    winner_definition = _optimized_definition(winner)
    ...
    winner_path = output_dir / "optimized_asset_pool_filter.json"
    ...
    winner_report = write_backtest_report(
        ...
        output_path=output_dir / "optimized_asset_pool_filter.html",
        database_path=database_path,
        split_date=split_date,
    ...
    if winner_report.warnings:
        (output_dir / "optimized_asset_pool_filter_report_warnings.txt").write_text(
    ...
    results = {
        "method": {
            ...
            "selectionWindow": f"{start_date.isoformat()} to {date(split_date.year - 1, 12, 31).isoformat()}",
            "objective": (
                "Average percentile rank of in-sample Sharpe, Calmar, annualized return, "
                "and information ratio versus same-universe multi-asset risk parity. OOS/IS Sharpe "
                f"retention closeness to {TARGET_RETENTION_LOW:.2f}-{TARGET_RETENTION_HIGH:.2f} is reported "
                "as a separate promotion score to avoid tuning parameters on the OOS window."
            ),
        ...
        "stabilityRankedCandidates": sorted(
            rows,
            key=lambda row: row["stabilityAdjustedPromotionScore"],
            reverse=True,
        )[:15],
    }
    ...
    with ProcessPoolExecutor(
        ...
        for future in as_completed(futures):
            ...
            if completed == 1 or completed % 10 == 0 or completed == len(tasks):
                print(f"completed {completed}/{len(tasks)} optimization cases with {workers} workers", flush=True)
    return rows
...
def _init_worker(
    ...
    _WORKER_SPLIT_DATE = date.fromisoformat(split_date)
...
def _run_candidate_task(task: tuple[int, dict[str, Any]]) -> dict[str, Any]:
    ...
    comparisons = _comparisons(
        ...
        split_date=_WORKER_SPLIT_DATE,
    )
    return _row(definition=definition, params=params, comparisons=comparisons)


def _candidate_definition(index: int, params: Mapping[str, Any]) -> Any:
    overlay = AssetPoolFilterOverlay(**params)
    return replace(
        strategy_definition_from_overlay(overlay),
        key=_case_key(params),
        name=_case_name(params),
        sleeve_name=f"optimized-candidate-{index:03d}",
        description="Grid candidate for pre-2023 multi-asset asset-pool filter optimization.",
    )
...
def _row(
    *,
    ...
    in_sample = vs_aor["metrics"]["in_sample"]["candidate"]
    oos = vs_aor["metrics"]["out_of_sample"]["candidate"]
    out_to_in = sharpe_retention_ratio(in_sample["sharpe"], oos["sharpe"])
    return {
        "key": definition.key,
        "name": definition.name,
        "parameters": _json_params(params),
        "inSampleAnnualizedReturn": in_sample["annualizedReturn"],
        "inSampleSharpe": in_sample["sharpe"],
        "inSampleCalmar": in_sample["calmar"],
        "inSampleMaxDrawdown": in_sample["maxDrawdown"],
        "inSampleInformationRatioVsMultiAssetRiskParity": vs_multi_rp["metrics"]["in_sample"]["active"]["informationRatio"],
        "inSampleInformationRatioVsCurrentSota": vs_sota["metrics"]["in_sample"]["active"]["informationRatio"],
        "inSampleInformationRatioVsAor": vs_aor["metrics"]["in_sample"]["active"]["informationRatio"],
        ...
        "outToInSharpeRatio": out_to_in,
        "outToInSharpeBandDistance": retention_band_distance(out_to_in),
        "outToInSharpeBandPass": retention_band_pass(out_to_in),
...
def _score_rows(rows: list[dict[str, Any]]) -> None:
    score_specs = [
        ("inSampleAnnualizedReturn", Decimal("0.20")),
        ("inSampleSharpe", Decimal("0.35")),
        ("inSampleCalmar", Decimal("0.25")),
        ("inSampleInformationRatioVsMultiAssetRiskParity", Decimal("0.20")),
    ]
    ...
    for row in rows:
        ...
        row["inSampleObjectiveScore"] = float(score)
    promotion_scores = {
        "oosSharpe": percentile_scores(rows, "oosSharpe"),
        "retention": retention_closeness_scores(rows, distance_key="outToInSharpeBandDistance"),
        "oosInformationRatioVsCurrentSota": percentile_scores(rows, "oosInformationRatioVsCurrentSota"),
    }
    for row in rows:
        key = row["key"]
        row["stabilityAdjustedPromotionScore"] = (
            0.45 * promotion_scores["oosSharpe"].get(key, 0.0)
            + 0.35 * promotion_scores["retention"].get(key, 0.0)
            + 0.20 * promotion_scores["oosInformationRatioVsCurrentSota"].get(key, 0.0)
        )
...
def _optimized_definition(winner: Mapping[str, Any]) -> Any:
    ...
    return replace(
        strategy_definition_from_overlay(overlay),
        key="optimized_multi_asset_asset_pool_filter",
        name="Optimized multi-asset asset-pool filter",
        sleeve_name="optimized-multi-asset-asset-pool-filter",
        state="research",
        description=(
            "Asset-pool ranking strategy selected by pre-2023 objective score, then frozen for OOS evaluation."
        ),
    )
...
def _results_markdown(results: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# Multi-Asset Pool Filter Optimization",
        "",
        f"- Candidate count: {results['method']['candidateCount']}",
        f"- Worker processes: {results['method']['workers']}",
        f"- Split date: {results['method']['splitDate']}",
        f"- Selection window: {results['method']['selectionWindow']}",
        f"- Objective: {results['method']['objective']}",
        "",
        "## Winner",
        "",
        f"- Strategy: {winner['name']}",
        f"- In-sample objective score: {_fmt_num(winner['inSampleObjectiveScore'])}",
        f"- In-sample Sharpe: {_fmt_num(winner['inSampleSharpe'])}",
        f"- In-sample Calmar: {_fmt_num(winner['inSampleCalmar'])}",
        f"- In-sample IR vs multi-asset risk parity: {_fmt_num(winner['inSampleInformationRatioVsMultiAssetRiskParity'])}",
        f"- OOS annualized return: {_fmt_pct(winner['oosAnnualizedReturn'])}",
        f"- OOS Sharpe: {_fmt_num(winner['oosSharpe'])}",
        f"- OOS/IS Sharpe ratio: {_fmt_num(winner['outToInSharpeRatio'])}",
        ...
        "",
        "## Top 15 By In-Sample Objective",
        "",
        "| Rank | Strategy | IS Score | IS Sharpe | OOS Sharpe | OOS/IS | Band Pass | OOS IR vs SOTA | OOS IR vs RP |",
        "| ---: | --- | ---: | ---: | ---: | ---: | --- | ---: | ---: |",
    ]
    ...
    lines.extend(
        [
            "",
            "## Top 15 By Stability-Adjusted Promotion Score",
            "",
            "| Rank | Strategy | Promotion Score | IS Sharpe | OOS Sharpe | OOS/IS | Band Pass | OOS IR vs SOTA |",
            "| ---: | --- | ---: | ---: | ---: | ---: | --- | ---: |",
        ]
    )
    for index, row in enumerate(results["stabilityRankedCandidates"], start=1):
        lines.append(
            "| "
            + " | ".join(
                [
                    str(index),
                    row["name"],
                    _fmt_num(row["stabilityAdjustedPromotionScore"]),
                    _fmt_num(row["inSampleSharpe"]),
                    _fmt_num(row["oosSharpe"]),
                    _fmt_num(row["outToInSharpeRatio"]),
                    "yes" if row["outToInSharpeBandPass"] else "no",
...
<head>
  ...
  <title>Optimized Multi-Asset Pool Filter</title>
...
<main>
  ...
  <p class="subtle">Parameters selected only from pre-2023 data, then frozen for the OOS window.</p>
  <section class="grid">
    <div class="stat"><span>IS Objective</span><strong>{_fmt_num(winner['inSampleObjectiveScore'])}</strong></div>
    <div class="stat"><span>IS Sharpe</span><strong>{_fmt_num(winner['inSampleSharpe'])}</strong></div>
    <div class="stat"><span>OOS Sharpe</span><strong>{_fmt_num(winner['oosSharpe'])}</strong></div>
    <div class="stat"><span>OOS/IS Sharpe</span><strong>{_fmt_num(winner['outToInSharpeRatio'])}</strong></div>
  </section>
  <section class="panel">
    ...
    *,
    ...
    return "\n".join(
        [
            ...
            f"- Optimization window: before {split_date.isoformat()}",
            f"- OOS window: {split_date.isoformat()} onward",
            f"- Candidate count: {results['method']['candidateCount']}",
            f"- Worker processes: {results['method']['workers']}",
            f"- Winner: {winner['name']}",
            f"- Winner OOS annualized return: {_fmt_pct(winner['oosAnnualizedReturn'])}",
            f"- Winner OOS Sharpe: {_fmt_num(winner['oosSharpe'])}",
            f"- Winner OOS/IS Sharpe ratio: {_fmt_num(winner['outToInSharpeRatio'])}",
            ...
            f"- Winner report: `{output_dir / 'optimized_asset_pool_filter.html'}`",
