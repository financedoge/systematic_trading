...
- Preparing the NAS implementation for the authorized push to `origin/master`. Preserve unrelated backfill edits, WSL/research notes and runtime reconciliation files outside this commit. Standard Git HTTPS failed with a connection reset; using the command-local Windows Schannel TLS backend succeeded without changing persistent Git settings.
- Validation: exact staged-code regression **356 passed, 1 skipped in 253.22s** from an isolated index export. PowerShell launcher parsing and staged whitespace checks passed. The skipped optional network smoke test is separate from the successful real-backup restore drill above. Code and backup are ready for publication; verify the pushed Git SHA against `origin/master` after committing.
